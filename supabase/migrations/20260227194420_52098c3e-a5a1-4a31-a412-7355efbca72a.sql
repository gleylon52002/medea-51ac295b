...
-- 1. Wallet user_id unique constraint
ALTER TABLE public.wallets ADD CONSTRAINT wallets_user_id_unique UNIQUE (user_id);

-- 2. Fix create_order_secure: replace currval with proper UUID capture for order_item_id
CREATE OR REPLACE FUNCTION public.create_order_secure(p_items jsonb, p_shipping_address jsonb, p_payment_method payment_method, p_shipping_cost numeric, p_notes text DEFAULT NULL::text, p_coupon_code text DEFAULT NULL::text, p_referral_code text DEFAULT NULL::text, p_wallet_amount numeric DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
